...
function FileChooser:genItemTable(...)
    ...
    if self.name == "filemanager" then
        ...
        if #item_table > 0 and item_table[1].is_go_up then
            self:_changeLeftIcon(
                BD.mirroredUILayout() and "back.top.rtl" or "back.top",
                function() self:onFolderUp() end
            )
            table.remove(item_table, 1)
        else
            self:_changeLeftIcon("home", self._left_tap_callback)
        end
    end
    return item_table
end
